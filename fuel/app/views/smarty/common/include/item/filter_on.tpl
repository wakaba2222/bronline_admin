<section class="filter" <!--{%* if $url == 'theme?'%}-->style="display:none;"<!--{%/if *%}-->>
	...
	<div class="wrap_filter">
		<ul class="filtered">
			<!--{%if $category%}-->
			<li><a href="/<!--{%$url%}-->" class="delete"><span></span><span></span></a> <!--{%$category%}--></li>
			<!--{%/if%}-->
			<!--{%foreach from=$sizes item=s%}-->
			<!--{%if $s%}-->
			<li><a href="javascript:delSizeSearch('<!--{%$s%}-->')" class="delete"><span></span><span></span></a> <!--{%$s%}--></li>
			<!--{%/if%}-->
			<!--{%/foreach%}-->

			<!--{%foreach from=$colors item=s%}-->
			<!--{%if $s%}-->
			<li><a href="javascript:delColorSearch('<!--{%$s%}-->')" class="delete"><span></span><span></span></a> <!--{%$s%}--></li>
			<!--{%/if%}-->
			<!--{%/foreach%}-->

			<!--{%foreach from=$brands item=s name=bb%}-->
			<!--{%if $s%}-->
			<li id="brandp<!--{%$smarty.foreach.bb.index%}-->"><a href="javascript:delBrandSearch('brandp<!--{%$smarty.foreach.bb.index%}-->')" class="delete"><span></span><span></span></a> <!--{%$s%}--></li>
			<!--{%/if%}-->
			<!--{%/foreach%}-->

			<!--{%foreach from=$shops key=k item=s%}-->
			<!--{%if $s%}-->
			<li><a href="javascript:delShopSearch('<!--{%$k%}-->')" class="delete"><span></span><span></span></a> <!--{%$s%}--></li>
			<!--{%/if%}-->
			<!--{%/foreach%}-->
		</ul>
		...
		<div class="item_menu">
			...
			<!--{%else%}-->
				...
				<ul style="display: block">
					<li><a href="javascript:addSearch('<!--{%$category%}-->')" <!--{%if $subcategory == ''%}-->class="active"<!--{%/if%}-->><!--{%$category%}--></a></li>
					<!--{%foreach from=$arrSubCategory item=cate%}-->
					<li><a href="javascript:addSearchSub('<!--{%$cate.name%}-->', 1)" <!--{%if $subcategory == $cate.name%}-->class="active"<!--{%/if%}-->><!--{%if $cate.another_name%}--><!--{%$cate.another_name%}--><!--{%else%}--><!--{%$cate.name%}--><!--{%/if%}--></a></li>
					<!--{%/foreach%}-->
					<li class="other_item">
						<div class="tit_item_menu">
							<p>その他カテゴリ</p>
...
function addShopSearch(word)
{
	...
	if (filters != '')
	{
		location.href = '/mall<!--{%$shop_url%}-->/brand?filters='+filters;
	}
	if (word == '')
		$('#shop').val('');
	else
