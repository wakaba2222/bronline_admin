			...
			<div class="item_menu">
				<div>
					...
					<ul style="display: block">
						<li class="all"><a href="/stylesnap/" class="<!--{%if $shop_url == ''%}--> active <!--{%/if%}-->" >ALL</a></li>
						<!--{%foreach from=$arrShop item=shop%}-->
							<!--{%if 0 < $arrShopArticleNum[$shop.login_id]%}-->
							<li><a href="/mall/<!--{%$shop.login_id%}-->/stylesnap/" class="<!--{%if $shop.login_id == $shop_url%}--> active <!--{%/if%}-->" ><!--{%$shop.shop_name%}--></a></li>
							<!--{%/if%}-->
						<!--{%/foreach%}-->
					</ul>
				</div>
