...
<!--{if $smarty.get.m == 'wear'}-->
function func_submit( id ){
    var fm = window.opener.document.form1;
    var no = escape('<!--{$smarty.get.no|h}-->');
    fm['wear_id' + no].value = id;
    fm.select_wear_no.value = no;
    fm.mode.value = 'wear_select';
    fm.anchor_key.value = 'wear_no' + no;
	fm.wear_shop_id.value = document.getElementById('shop_mode').value;
    fm.submit();
    window.close();
    return false;
}
...
function func_submit( id ){
	...
	fm.recomment_shop_id.value = document.getElementById('shop_mode').value;
//	alert(fm.shop.value);
    fm.submit();
    window.close();
    return false;
