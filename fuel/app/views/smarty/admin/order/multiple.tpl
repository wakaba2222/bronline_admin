...
$(function() {
    ...
    <!--{*## 追加規格 ADD BEGIN ##*}-->
    var extra_info = window.opener.jQuery('input[id^=extra_info]');
    <!--{*## 追加規格 ADD END ##*}-->
    ...
    for (var i = 0; i < product_class_id.length; i++) {
        ...
        for (var j = 0; j < $(quantity[i]).val(); j++) {
            ...
            <!--{*## 追加規格 ADD BEGIN ##*}-->
            // 追加規格
            var extra_info_nm = window.opener.jQuery('input[id^=extra_info_nm'+i+']');
            var extra_info_ctg = window.opener.jQuery('input[id^=extra_info_ctg'+i+']');
            var extra_info_idx = 0;
            var extrafield = '';
            $(extra_info_nm).each(function(){
              extrafield += '<input type="hidden" name="multiple_extra_info[' + index + '][' + extra_info_idx + '][extra_class_name]" value="' + $(this).val() + '" />';
              extrafield += '<input type="hidden" name="multiple_extra_info[' + index + '][' + extra_info_idx + '][extra_classcategory_name]" value="' + $(extra_info_ctg[extra_info_idx]).val() + '" />';
              dispname += '<br />' + $(this).val() + '： ' + $(extra_info_ctg[extra_info_idx]).val();
              extra_info_idx++;
            });
            <!--{*## 追加規格 ADD END ##*}-->
