...
//    fm1.getElementById(opner_shop_name).value = fm1.getElementById('shop_mode');
    fm1.getElementById('shop_mode').value = document.getElementById('shop_mode').value;
...
arrAllExtraClass[<!--{$key}-->] = "<!--{$item.name}-->";
<!--{/foreach}-->
// 追加規格設定
function fnSetExtraCls(product_id, doc){
  ...
  if(arrExtclsId.length > 0){
    for(i in arrExtclsId){
      ...
      if(openerVal == ""){
        openerVal += arrAllExtraClass[id] + ":" + extcls_val;
      } else{
        openerVal += "," + arrAllExtraClass[id] + ":" + extcls_val;
      }
    }
...
<table class="form">
    ...
    <tr>
        <th>ショップ</th>
        <td>
            <select name="search_shop_id" id="shop_mode">
                <option value="" selected="selected">BR</option>
                <!--{html_options options=$arrShopList selected=$arrForm.search_shop_id}-->
            </select>
        </td>
    </tr>
    <tr>
                ...
                <tr style="background:<!--{$arrPRODUCTSTATUS_COLOR[$status]}-->;">
                    ...
                    <td>
                        ...
                        <!--{section name=extcnt loop=$arrExtraClass}-->
                            ...
                            <!--{assign var=extraClsName value=$arrAllExtraClass[$extraClsId].name}-->
                            ...
                            <dl>
                                ...
                                <dd>
                                    <!--{if $arrAllExtraClass[$extraClsId].url == ''}-->
                                    <select name="<!--{$key}-->" id="<!--{$key}-->" style="<!--{$arrErr[$key]|sfGetErrorColor}-->">
                                        <!--{html_options values=$extraClsCat output=$extraClsCat selected=$arrForm[$key]}-->
                                    </select>
                                    <!--{else}-->
                                    <input type="text" id="<!--{$key}-->" name="kikaku_input<!--{$extraClsId}-->" size="10" value="" />
                                    <!--{/if}-->
