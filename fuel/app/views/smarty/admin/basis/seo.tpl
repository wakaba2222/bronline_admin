...
<!--{*
                            <tr>
                                <th>OGタグ:Description</th>
                                <td>
                                <span class="attention"><!--{$arrErr[$item.device_type_id][$item.page_id].og_description}--></span>
                                <input type="text" name="meta[<!--{$item.device_type_id}-->][<!--{$item.page_id}-->][og_description]" value="<!--{$item.og_description|h}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" size="60" class="box60" style='<!--{if $arrErr[$item.page_id].og_description != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->' /><span class="attention"> (上限<!--{$smarty.const.STEXT_LEN}-->文字)</span></td>
                            </tr>
                            <tr>
                                <th>OGタグ:Keyword</th>
                                <td>
                                <span class="attention"><!--{$arrErr[$item.device_type_id][$item.page_id].og_keyword}--></span>
                                <input type="text" name="meta[<!--{$item.device_type_id}-->][<!--{$item.page_id}-->][og_keyword]" value="<!--{$item.og_keyword|h}-->" maxlength="<!--{$smarty.const.STEXT_LEN}-->" size="60" class="box60" style='<!--{if $arrErr[$item.page_id].og_keyword != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->' /><span class="attention"> (上限<!--{$smarty.const.STEXT_LEN}-->文字)</span></td>
                            </tr>
*}-->
                        </table>
