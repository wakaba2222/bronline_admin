...
*}-->
    ...
    <table id="basis-index-func">
        ...
        <input type="hidden" name="downloadable_days" value="1" />
<!--{*
        <tr>
            <th>新着表示期間</th>
            <td>
                <span class="attention"><!--{$arrErr.new_rule}--></span>
                <input type="text" name="new_rule" value="<!--{$arrForm.new_rule|default:"7"|h}-->" maxlength="<!--{$smarty.const.INT_LEN}-->" size="6" class="box6" style="<!--{if $arrErr.new_rule != ""}-->background-color: <!--{$smarty.const.ERR_COLOR}-->;<!--{/if}-->" /> 日間　新着表示
            </td>
        </tr>
        <tr>
            <th>ダウンロード可能日数</th>
